  ...
  for (let i in SearchSourceDocument) {
    if (inputValue.Source_Document_Type?.id?.display_name === "Ist") {
      ...
      options1 = options1.concat(
        ...
        {
          value: SearchSourceDocument[i]?.ist_number,
          id: SearchSourceDocument[i]?.id,
        }
      );
      ...
      options1 = options1.concat(
        ...
        {
          value: SearchSourceDocument[i]?.asn_number,
          id: SearchSourceDocument[i]?.id,
        }
      );
    } else if (
      inputValue.Source_Document_Type?.id?.display_name === "Sales Order"
    ) {
      options1 = options1.concat(
        // SearchSourceDocument[i]?.sales_order_number
        {
          value: SearchSourceDocument[i]?.sales_order_number,
          id: SearchSourceDocument[i]?.id,
        }
      );
    } else if (
      inputValue.Source_Document_Type?.id?.display_name === "Purchase return"
    ) {
      options1 = options1.concat(
        // SearchSourceDocument[i]?.purchase_return_number
        {
          value: SearchSourceDocument[i]?.purchase_return_number,
          id: SearchSourceDocument[i]?.id,
        }
      );
    } else if (
      inputValue.Source_Document_Type?.id?.display_name === "Purchase order"
    ) {
      options1 = options1.concat(
        // SearchSourceDocument[i]?.purchase_order_number
        {
          value: SearchSourceDocument[i]?.purchase_order_number,
          id: SearchSourceDocument[i]?.id,
        }
      );
    } else {
      options1 = options1.concat(
        ...
        {
          value: SearchSourceDocument[i]?.number,
          id: SearchSourceDocument[i]?.id,
        }
      );
    }
