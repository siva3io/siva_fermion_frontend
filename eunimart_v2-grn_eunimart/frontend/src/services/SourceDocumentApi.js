...
export const config = (searchValue) => {
  ...
  if (searchValue.lookup_code == "ASN") {
    console.log("Asn1");
    url = `${base_URL}/api/v1/asn`;
  }

  if (searchValue.display_name === "Purchase return") {
  ...
  if (searchValue.display_name == "Sales Order") {
    url = `${base_URL}/api/v1/sales_orders/dropdown`;
  }
  if (searchValue.display_name == "Ist") {
