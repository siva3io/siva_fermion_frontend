...
const ProductLineItems = grnData => {
  ...
  const products = grnData?.grnData?.grn_order_lines
    ? grnData?.grnData?.grn_order_lines[0]
      ? grnData?.grnData?.grn_order_lines[0]
      : []
    : [];
            ...
            return {
              product_name:
                item && item.product && item.product.product_name
                  ...
                  : "--",
              sku_id:
                item && item.product && item.product.sku_id
                  ? item.product.sku_id
                  : "--",
              ...
              ordered_units:
                item && item.ordered_units ? item.ordered_units : 0,
              received_units:
                item && item.received_units ? item.received_units : 0,
              pending_units:
                item && item.pending_units ? item.pending_units : 0,
              quality_check: item && item.quality_check ? "true" : "false",
              shelf_location:
                item && item.shelf_location ? item.shelf_location : 0,
              rejected_quantities:
                item && item.rejected_quantities ? item.rejected_quantities : 0,
              reason_of_rejection:
                item && item.reason_of_rejection ? item.reason_of_rejection : 0,
            };
          })}
