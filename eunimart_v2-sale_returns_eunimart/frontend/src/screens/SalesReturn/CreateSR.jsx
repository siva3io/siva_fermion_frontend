...
import { DocumentData } from "../../redux/Action/SalesReturns/DocumentData";
...
const ASNCreate = () => {
    ...
    const [selectedProductData, setSelectedProductData] = useState([{ Quantity: 0, selling_price: 0, discount: 0}]);
    ...
    const onAddNewRaw = () => {
        setSelectedProductData([...selectedProductData, { Quantity: 0, selling_price: 0, discount: 0 }]);
    }
    ...
    console.log(Returnlocation, "locationsreturn");
    ...
    const handelInputChange = (key, value, index = null) => {
        ...
        if (key == "Schedule_Pickup_time_from" || key == "Schedule_Pickup_time_to") {
            ...
            if (key == "Package_Length" || key == "Package_Weight" || key == "Package_Width" || key == "Vol_Weight" || key == "Package_Height") {
        ...
        if (index != null) {
            ...
            if (key === 'sku_id') {
                ...
                console.log(key)
                if (key.toString().includes('.')) newSelectedProductData[index][key.split(".")[1]] = value;
            ...
            var tax = 0;
            if (newSelectedProductData[index].product_pricing_details && newSelectedProductData[index].product_pricing_details?.tax_options && parseFloat(newSelectedProductData[index].product_pricing_details1?.tax_options) > 0) {
            ...
            setSelectedProductData(newSelectedProductData);
            // var x = newSelectedProductData[0]?.product_pricing_details?.tax_options ? newSelectedProductData[0]?.product_pricing_details?.tax_options : newSelectedProductData[index]?.tax
    ...
    const handelSelectonChange = (key, value) => {
        ...
        switch (key) {
            ...
            case "Link_Document_Type": setSRDetailsFields(SRDetailsFields?.map(o => { if (o.key == key) return { ...o, value: value }; return o }));
                break;
    ...
    const handleButtonClick = (key) => {
        ...
        const payload = {
            ...
            "so_id": 1, //backend need to remove
            "do_id": 1, //backend need to remove
            ...
            "customer_pickup_address": {
                ...
                state: mainData.ShippingAddress_State?.label ? mainData.ShippingAddress_State?.label : mainData.ShippingAddress_State,
                country: mainData.ShippingAddress_Country?.label ? mainData.ShippingAddress_Country?.label : mainData.ShippingAddress_Country
            },
            ...
            "sales_return_number": mainData.SR_Number,
            "source_document_id":mainData?.Link_Document_Type?.id,
            "source_document": {
              "id":mainData?.Link_Documents?.id ,
              "data": mainData?.Link_Documents?.label
            },
            // "status_history": {
            // },
            //added
            ...
            "sr_payment_details": {
                ...
                "adjustments": 100,  //added
            },
            "pickup_date_and_time": {
                "pickup_date": "2022-05-12T22:19:32.8080397+05:30",
                "pickup_from_time": "2022-05-12T22:19:32.8080397+05:30",
                "pickup_to_time": "2022-05-12T22:19:32.8080397+05:30"
            },
        ...
        dispatch(createSalesReturnsList(payload));
    ...
    const [CreditNote1, setCreditNote1] = useState([
        {
            ...
            type: "radio",
            key: "Credit_Method",
            sub: [{ "label": "Create credit on reciept", "value": 2, "checked": true }, { "label": "Issue CN directly", "value": 3}],
        },
    ...
    const handelRadionButtononChange = (prop, value) => {
        ...
        if (prop == "Credit_Method") {
            ...
            var newState = CreditNote1?.map(o => { if (o.key == prop) o.sub?.map(p => { if (p.value == value) p.checked = true; return p; }); return o })
    ...
    const handelCheckboxShowForCopyField_valueChange = (field) => {
        console.log("onCheckboxChanges1", field);
        if(field=="same as shipping address"){
            setCheckboxShowForCopyField_value(!CheckboxShowForCopyField_value);
        }
        if(field=="Create Credit Note"){
            setCreditCheck(!creditCheck);
        }
        if(field=="Create Shipping"){
            setCreateShipping(!createShipping);
        }
    ...
    return (
        <>
            ...
            <AddForm header={"Credit Details"} data={CreditNote1} handelInputChange={handelInputChange} handelSelectonChange={handelSelectonChange} handelCheckBoxonChange={handelCheckBoxonChange} handelRadionButtononChange={handelRadionButtononChange} IsCheckboxShowForCopyField={true} CheckboxShowForCopyField_text={"Create Credit Note"} CheckboxShowForCopyField_value={creditCheck} handelCheckboxShowForCopyField_valueChange={handelCheckboxShowForCopyField_valueChange} Islabel_priceSpace={true} />

            {/* //Shipping Details */}

            <AddForm header={"Shipping Details"} data={ShippingDetailsFields} handelInputChange={handelInputChange} handelSelectonChange={handelSelectonChange} handelCheckBoxonChange={handelCheckBoxonChange} handelRadionButtononChange={handelRadionButtononChange} IsCheckboxShowForCopyField={true} CheckboxShowForCopyField_text={"Create Shipping"} CheckboxShowForCopyField_value={createShipping} handelCheckboxShowForCopyField_valueChange={handelCheckboxShowForCopyField_valueChange} Islabel_priceSpace={true} />
