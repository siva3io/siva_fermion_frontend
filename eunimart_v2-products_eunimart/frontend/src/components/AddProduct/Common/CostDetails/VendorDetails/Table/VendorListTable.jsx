...
export default function VendorListTable({
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked && vendors) {
      const newSelecteds = vendors.data.map((n) => n.name);
  ...
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
    if (vendors && vendors.meta) {
      setParams({
        limit: Number(vendors.meta.info.per_page),
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Paper sx={{ width: "100%" }}>
        ...
        <TableContainer sx={{ minHeight: "46vh", p: 2 }}>
          <Table
            sx={{ minWidth: 750 }}
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={vendors && vendors.data && vendors.data.length}
            />
            <TableBody>
        ...
        {/* <TablePagination
          ...
          component="div"
          count={vendors && vendors.meta && vendors.meta.info.total_rows}
          rowsPerPage={vendors && vendors.meta && vendors.meta.info.per_page}
          page={page}
