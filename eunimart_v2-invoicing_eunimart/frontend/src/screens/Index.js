...
import { loadPruchaseInvoiceData, loadDeleteDataById ,viewPIAccessManagement} from "../redux/action";
...
const Index = () => {  
  ...
  const { PurchaseInvoicedata, PurchaseInvoicedata_meta, Deletedata, piAccess } = useSelector((state) => state.data);
  useEffect(() => {
  ...
  const sortOptions = [
    {
      ...
      func: (value) => {
        dispatch(loadPruchaseInvoiceData({ limit: params.limit, offset: params.offset, filters:params.filters, sort:JSON.stringify([["purchase_invoice_number",value]]) }))
      },
    },
    {
      label: "Reference Number",
      ...
      func: (value) => {        
        dispatch(loadPruchaseInvoiceData({ limit: params.limit, offset: params.offset, filters:params.filters, sort:JSON.stringify([["reference_number",value]]) }))
      },
    },
    {
      label: "Clear All",
  ...
  const [customOptions, setCustomOptions] = useState([
    {
      ...
      flag:piAccess?.find(row=>row === row)?.view_actions_json?.find(o=>o.lookup_code === "READ" )?.ctrl_flag
    },
    {
      label: "Edit",
  ...
  useEffect(()=>{
    setCustomOptions(
      ...
      },
      {
        ...
        flag:piAccess?.find(row=>row === row)?.view_actions_json?.find(o=>o.lookup_code === "UPDATE" )?.ctrl_flag

      },
      {
        label: "Delete",
        func: (product_id) => handleDeleteModalOpen(product_id),
        flag:piAccess?.find(row=>row === row)?.view_actions_json?.find(o=>o.lookup_code === "DELETE" )?.ctrl_flag
...
const handleDeleteModalOpen = (value) => {
  ...
  const filterSearchItems = (searchValue, searchType) => {
    if (searchValue.length === 0) { 
      ...
      dispatch( 
        dispatch(loadPruchaseInvoiceData({ limit: params.limit, offset: params.offset, filters:JSON.stringify([[searchType,"ilike",searchValue]]) }))
      );
    }
  };
...
{
          PurchaseInvoicedata.length > 0 && piAccess && piAccess[0]?.module_ctrl_flag && piAccess?.find(row=>row === row)?.view_actions_json?.find(o=>o.lookup_code === "LIST" )?.ctrl_flag ===1 && (
    ...
    <Suspense fallback={<div>Loading... </div>}>      
        <RemoteDynamicAppBar
          ...
          buttons={[{name:"Create",handleButtonClick:handleButtonClick,flag:piAccess?.find(row=>row === row)?.view_actions_json?.find(o=>o.lookup_code === "CREATE" )?.ctrl_flag}]}
