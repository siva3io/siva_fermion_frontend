...
import { loadTableContact, deleteContact, viewAccessManagement } from "../redux/Action/action";
...
const ContactsIndex = () => {
  ...
  const { contacts_table_data, contactsdata_table_meta, contactsdata, access } = useSelector((state) => state.data);
  ...
  return (
    contacts_table_data.length > 0 && (
      <div>
        ...
        {contacts_table_data.length > 0 && access && access[0]?.module_ctrl_flag && (
          <div>
            <RemoteDynamicTable
